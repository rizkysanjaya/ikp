...
<div x-data="{ 
    ...
    openEdit(item) {
        this.isEdit = true;
        this.form = { id: item.id, nama_pekerjaan: item.nama_pekerjaan };
        this.showModal = true;
    }
}">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Master Data Pekerjaan</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola referensi pekerjaan responden</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 text-gray-600 text-sm uppercase tracking-wider border-b border-gray-200">
                        ...
                        <th class="px-6 py-4 font-semibold">Nama Pekerjaan</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php foreach ($pekerjaan as $index => $item) : ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-gray-500 text-sm text-center"><?= $index + 1 ?></td>
                            <td class="px-6 py-4 text-gray-800 font-medium"><?= esc($item->nama_pekerjaan) ?></td>
                            ...
                            <td class="px-6 py-4 text-center space-x-1">
                                <a href="<?= base_url('admin/master/pekerjaan/toggle/' . $item->id) ?>" class="inline-flex items-center justify-center p-2 rounded-lg transition-colors <?= $item->is_active ? 'text-orange-500 hover:bg-orange-50' : 'text-green-500 hover:bg-green-50' ?>" title="<?= $item->is_active ? 'Nonaktifkan' : 'Aktifkan' ?>">
                                ...
                                <a href="<?= base_url('admin/master/pekerjaan/delete/' . $item->id) ?>" onclick="return confirm('Yakin ingin menghapus data ini?')" class="inline-flex items-center justify-center p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <?php if (empty($pekerjaan)) : ?>
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500">Belum ada data pekerjaan.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full relative z-10 transform transition-all">
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-gray-800" x-text="isEdit ? 'Edit Pekerjaan' : 'Tambah Pekerjaan'"></h3>
